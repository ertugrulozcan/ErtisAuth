...
namespace ErtisAuth.Hub.Services
{
	public class SessionService : ISessionService
	{
		...
		public SessionService(IHttpContextAccessor httpContextAccessor, IAuthenticationService authenticationService, ILogger<SessionService> logger)
		{
		...
		public SessionUser GetSessionUser()
		{
			...
			var photoUrl = this.httpContextAccessor.HttpContext.GetClaim(Claims.PhotoUrl);
		...
		public async Task<IResponseResult<User>> StartSessionAsync(HttpContext httpContext, BearerToken bearerToken)
		{
			// Who am i?
            var meResult = await this.authenticationService.WhoAmIAsync(BearerToken.CreateTemp(bearerToken.AccessToken));
            if (meResult.IsSuccess)
            {
            	var claims = new List<Claim>
            	{
            		...
            		new Claim(Claims.PhotoUrl, string.Empty),
