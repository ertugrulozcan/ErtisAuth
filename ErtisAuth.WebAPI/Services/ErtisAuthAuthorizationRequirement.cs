...
namespace ErtisAuth.WebAPI.Services
{
	...
	public class ErtisAuthAuthorizationHandler : AuthorizationHandler<ErtisAuthAuthorizationRequirement>
	{
		...
		private async Task CheckAuthorizationAsync(HttpContext httpContext)
		{
			...
			var verifyTokenResult = await this.tokenService.VerifyTokenAsync(token, false);
			...
			if (!string.IsNullOrEmpty(user.Role))
			{
				...
				if (role != null)
				{
					...
					// Subject
					var rbacSubjectSegment = new RbacSegment(user.Id);
					...
					{
						throw ErtisAuthException.AccessDenied("User role is unauthorized for this action");
					}
				}
			}
		}
